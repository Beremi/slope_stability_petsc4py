#!/usr/bin/env python
"""Run 2D asset-backed mechanics cases through the PETSc stack."""
...
from slope_stability.assets.evaluators import saturation_from_hydraulic_state
...
def run_capture(
    ...
    *,
    ...
    if material_rows is None:
        raise ValueError(f"Mechanical material rows are required for asset {asset_name!r}.")
    ...
    hydraulic_state = resolved_asset.definition.hydraulic_state()
    if seepage_spec is None and hydraulic_state is not None:
        hatp, _, _ = local_basis_volume_2d(elem_type, xi)
        saturation = saturation_from_hydraulic_state(coord, elem, hatp, hydraulic_state)
    elif seepage_spec is None:
        saturation = np.ones(n_int, dtype=bool)
    else:
        grho = float(seepage_spec.seepage.water_unit_weight)
        conduct0 = heter_conduct(material_identifier, n_q, np.asarray(seepage_spec.conductivity, dtype=np.float64))
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run a 2D asset-backed mechanics continuation case.")
