...

{
  ...
  find "$RESULTS_DIR" -mindepth 1 -maxdepth 1 -type d | sort | while read -r dir; do
    ...
    if [[ -n "$job_id" ]] && command -v sacct >/dev/null 2>&1; then
      sacct --units=M -j "$job_id" --format=JobID,JobName%40,State,Elapsed,AllocNodes,AllocCPUS,NTasks,MaxRSS,AveRSS,MaxVMSize,ExitCode -P >"$sacct_file" 2>/dev/null || true
    fi
