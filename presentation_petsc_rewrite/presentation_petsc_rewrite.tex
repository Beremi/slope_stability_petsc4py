...
\begin{pseudobox}
start\\
$\rightarrow$ load config $\rightarrow$ choose 3D SSR runner $\rightarrow$ load and reorder mesh\\
...
\midrule
Select the runner family & pick a top-level script & copy a benchmark folder and set \code{problem.case} \\
...
Point at the mesh input & edit the loader call inside the script & use \code{problem.mesh\_path} for mesh families or \code{[case\_data].mesh\_dir} in 2D text-mesh cases \\
Define heterogeneity & edit material and conductivity arrays in the script & inherit from \code{meshes/*/definition.py}, override with \code{[[materials]]}, keep hydraulic data in \code{[seepage]} \\
...
\midrule
\code{problem.case} & \code{3d\_hetero\_ssr}, \code{3d\_hetero\_seepage}, \code{2d\_franz\_dam\_ssr} & the concrete capture runner family and its mesh convention \\
\code{problem.analysis} & \code{ssr}, \code{ll}, \code{seepage} & the dry SSR path, limit-load path, or seepage-only path inside that family \\
\code{continuation.method} & \code{indirect}, sometimes \code{direct} & only meaningful where the chosen runner exposes both modes; the current 3D mainline families discussed here are indirect \\
\code{problem.mesh\_path} & family-local \code{.msh} file & the concrete mesh file for 3D and other Gmsh-based families \\
\code{[case\_data].mesh\_dir} & \code{meshes/2d\_franz\_dam} & the mesh source for the 2D text-mesh families \\
\code{problem.seepage} & \code{true}, \code{false} & a case flag; it does not by itself select a seepage runner \\
\bottomrule
\end{tabularx}
\end{frame}
...
\toprule
\textbf{Family style} & \textbf{Analysis} & \textbf{Current continuation surface} & \textbf{Mesh input} \\
\midrule
2D text-mesh families & \code{ssr}, \code{ll} & indirect everywhere; direct available here & \code{[case\_data].mesh\_dir} \\
3D dry families & \code{ssr}, \code{ll} & indirect in the current config-driven mainline & \code{problem.mesh\_path} plus family metadata \\
3D seepage-only & \code{seepage} & no continuation branch; seepage and linear-solver blocks drive the solve & \code{problem.mesh\_path} on the seepage mesh family \\
3D seepage-coupled SSR families & \code{ssr} & indirect in the current families; hydro is activated with \code{problem.seepage = true} & dedicated seepage-SSR family \\
\bottomrule
\end{tabularx}
\vspace{0.12cm}
\begin{callout}
\code{problem.analysis} is public. Which other knobs matter still depends on the chosen runner family.
\end{callout}
\end{frame}
...
\code{analysis = "ssr"}\\
\code{elem\_type = "P4"}\\
\code{mesh\_path = "<hetero SSR mesh>.msh"}\\
\code{mesh\_boundary\_type = 0}\\[0.10cm]
\code{[execution]}\\
...
\begin{itemize}
  ...
  \item Because \code{mesh\_path} lives under \code{meshes/3d\_hetero\_ssr/}, the run can inherit that family's BC labels and default materials.
...
\begin{pseudobox}
\code{[problem]}\\
\code{case = "3d\_hetero\_seepage"}\\
\code{analysis = "seepage"}\\
\code{mesh\_path = "<seepage mesh>.msh"}\\[0.10cm]
\code{[seepage]}\\
\code{conductivity = [ ... ]}\\
...
\begin{pseudobox}
\code{[problem]}\\
\code{case = "3d\_hetero\_seepage\_ssr\_comsol"}\\
\code{analysis = "ssr"}\\
\code{seepage = true}\\[0.10cm]
\code{[continuation]}\\
\code{method = "indirect"}\\[0.10cm]
\code{[seepage]}\\
\code{conductivity = [ ... ]}
\end{pseudobox}
\end{column}
...
\vspace{0.12cm}
\begin{callout}
\code{[seepage]} carries conductivity and seepage solver tolerances. Waterlevels versus COMSOL boundary handling is still selected by the seepage runner family and mesh convention, not by a generic TOML field.
\end{callout}
\end{frame}
...
\code{...}\\[0.10cm]
\code{[case\_data]}\\
\code{mesh\_dir = "../../meshes/2d\_franz\_dam"}
\end{pseudobox}
\end{column}
\begin{column}{0.48\textwidth}
\begin{softbox}
\begin{itemize}
  \item If \code{mesh\_path} lives under \code{meshes/<family>/}, the asset loader can reuse that family's \code{dirichlet\_labels} and default materials.
  \item The IO layer builds mechanical Dirichlet masks from those labels; if no family metadata exists, it falls back to default axis-label sets.
  \item \code{mesh\_boundary\_type = 1} is the 3D glued-bottom special case.
  \item \code{[[materials]]} overrides constitutive rows locally; \code{[seepage].conductivity} stays separate as hydraulic input.
  \item 2D text-mesh cases use \code{[case\_data].mesh\_dir}; direct SSR is exposed there, not on the current 3D mainline.
\end{itemize}
\end{softbox}
\end{column}
\end{columns}
...
\begin{enumerate}
  ...
  \item Set \code{[problem]} first: \code{case}, \code{analysis}, \code{elem\_type}, and either \code{mesh\_path} or \code{[case\_data].mesh\_dir}.
  \item Use \code{continuation.method} only if the chosen runner family exposes both indirect and direct paths.
  \item Decide whether BC labels and constitutive rows should come from \code{meshes/*/definition.py} or from benchmark-local overrides such as \code{[[materials]]}.
  \item Fill \code{[execution]}, \code{[continuation]}, \code{[newton]}, and \code{[linear\_solver]}; add \code{[seepage]} only when the case has a hydraulic block.
...
\vspace{0.14cm}
\begin{callout}
A benchmark should remain explainable from \code{case.toml}, the mesh-family definition, and the output folder. If that is no longer true, too much logic has drifted back into hidden runner code.
\end{callout}
\end{frame}
...
Runner-side assembly and problem wiring & the relevant capture runner under \code{src/slope\_stability/cli/} \\
