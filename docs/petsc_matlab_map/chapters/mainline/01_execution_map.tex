...
\code{benchmarks/slope\_stability\_3D\_hetero\_SSR\_default/case.toml}. In that benchmark the
main path is:
\[
...
\item \code{asset = 3d\_hetero\_slope},
\item \code{mesh\_variant = adaptive\_family\_a\_l1.msh},
...
stages:
\begin{enumerate}
\item \petscmod{cli/run\_case\_from\_config.py} receives the case config.
\item \petscmod{core/run\_config.py} loads the TOML, derives dimension from the
asset, and rejects legacy problem-owned fields such as raw mesh paths.
\item \petscmod{problem\_asset\_runtime.py} resolves the asset, mesh variant,
and profile.
\item \petscmod{execution/asset\_case/runner.py} selects the generic 3D
mechanics route.
\item \petscmod{execution/asset\_case/mechanics\_3d.py::run\_capture}
performs mesh construction, constitutive-builder setup, solver construction,
and continuation launch.
...
\begin{enumerate}
\item resolve asset \code{3d\_hetero\_slope} and mesh variant
\code{adaptive\_family\_a\_l1.msh},
\item build the promoted solver mesh from the canonical asset definition,
...
Mesh and material preprocessing & script body plus \matlabpkg{+MESH/*} and \matlabpkg{+ASSEMBLY/*} & \petscmod{problem\_asset\_runtime.py}, \petscmod{execution/asset\_case/mechanics\_3d.py}, \petscmod{mesh/*}, \petscmod{fem/*} \\
...
\begin{enumerate}
\item \petscmod{cli/run\_case\_from\_config.py} and
\petscmod{execution/asset\_case/runner.py} for orchestration,
\item \petscmod{execution/asset\_case/mechanics\_3d.py} for 3D mechanics setup,
