...
DEFAULT_OUT_ROOT = ROOT / "artifacts" / "p4_pmg_galerkin"
DEFAULT_REPORT = SCRIPT_DIR / "report_p4_pmg_galerkin.md"
...
def _run(spec: RunSpec, *, out_root: Path) -> dict[str, object]:
    ...
    if run_info is not None:
        ...
        if spec.stage == "micro":
            pmg_micro = run_info.get("pmg_microbenchmark") or {}
            if isinstance(pmg_micro, dict):
                metrics.update(
                    {
                        "pc_apply_elapsed_s": pmg_micro.get("pc_apply_elapsed_s"),
                        "coarse_ksp_type": pmg_micro.get("pmg_coarse_ksp_type"),
                        "coarse_pc_type": pmg_micro.get("pmg_coarse_pc_type"),
                        "coarse_subcomm_size": pmg_micro.get("pmg_coarse_subcomm_size"),
                        "coarse_iterations": pmg_micro.get("pmg_coarse_iterations"),
                        "coarse_converged_reason": pmg_micro.get("pmg_coarse_converged_reason"),
                    }
                )
...
def _frozen_specs(python: str, out_root: Path) -> list[RunSpec]:
    probe = ROOT / "benchmarks" / "slope_stability_3D_hetero_SSR_default" / "archive" / "probe_hypre_frozen.py"
    common = (
        "--state-selector",
        "hard",
        "--outer-solver-family",
    ...
    for ranks in (1, 8):
        ...
        specs.append(
            RunSpec(
                name=f"rank{ranks}_pmg_galerkin",
                stage="linear",
                ranks=ranks,
                timeout_s=1800,
                command=(
                    *mpiexec,
                    ...
                    str(probe),
                    "--out-dir",
                    str(out_root / "linear" / f"rank{ranks}_pmg_galerkin"),
                    "--state-npz",
                    str(state_npz),
                    "--state-run-info",
                    str(state_run_info),
...
def _micro_specs(python: str, out_root: Path) -> list[RunSpec]:
    probe = ROOT / "benchmarks" / "slope_stability_3D_hetero_SSR_default" / "archive" / "probe_hypre_frozen.py"
    state_npz = STATE_ROOT / "rank8" / "data" / "petsc_run.npz"
    state_run_info = STATE_ROOT / "rank8" / "data" / "run_info.json"
    return [
        RunSpec(
            name="rank8_pmg_galerkin_micro",
            stage="micro",
            ranks=8,
            timeout_s=900,
            command=(
                ...
                str(probe),
                "--out-dir",
                str(out_root / "micro" / "rank8_pmg_galerkin_micro"),
                "--state-npz",
                str(state_npz),
                "--state-run-info",
                str(state_run_info),
    ...
    return [
        ...
        RunSpec(
            ...
            command=(
                ...
                runner,
                "--out_dir",
                str(out_root / "nonlinear" / "rank8_pmg_galerkin_step2"),
                "--pc_backend",
                "pmg",
                "--preconditioner_matrix_source",
                "tangent",
...
def _write_report(*, entries: list[dict[str, object]], report_path: Path, summary_path: Path) -> None:
    lines = [
        ...
        "## PMG Microbenchmark",
        "",
        "| Run | Status | Backend | Setup [s] | PCApply [s] | Coarse KSP | Coarse PC | Coarse subcomm | Coarse iters |",
        "| --- | --- | --- | ---: | ---: | --- | --- | ---: | ---: |",
    ]
    micro_entries = [entry for entry in entries if entry["stage"] == "micro"]
    for entry in micro_entries:
        metrics = entry.get("metrics", {})
        lines.append(
            "| {name} | {status} | {backend} | {setup} | {apply} | {coarse_ksp} | {coarse_pc} | {subcomm} | {iters} |".format(
                name=entry["name"],
                status=entry["status"],
                backend=metrics.get("pc_backend", "-"),
                setup=("{:.3f}".format(metrics["setup_elapsed_s"]) if metrics.get("setup_elapsed_s") is not None else "-"),
                apply=("{:.3f}".format(metrics["pc_apply_elapsed_s"]) if metrics.get("pc_apply_elapsed_s") is not None else "-"),
                coarse_ksp=metrics.get("coarse_ksp_type", "-"),
                coarse_pc=metrics.get("coarse_pc_type", "-"),
                subcomm=metrics.get("coarse_subcomm_size", "-"),
                iters=metrics.get("coarse_iterations", "-"),
            )
        )
    ...
    if skipped_entries:
        ...
        for entry in skipped_entries:
            lines.append(f"- `{entry['name']}`: {entry.get('reason', '-')}")

    report_path.write_text("\n".join(lines) + "\n", encoding="utf-8")


def main() -> None:
    parser = argparse.ArgumentParser(description="Run the P4 tangent/Galerkin PMG comparison suite.")
    ...
    for spec in _micro_specs(python, out_root):
