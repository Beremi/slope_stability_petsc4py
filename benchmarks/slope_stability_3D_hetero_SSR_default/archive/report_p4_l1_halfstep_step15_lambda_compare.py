...
OUT_DIR = Path("artifacts/p4_l1_halfstep_step15_lambda_compare/report")
CASE_SPECS = (
    {
        "name": "secant_halfstep_step30_prefix15",
        "label": "Secant, Half Step",
        "progress": Path("artifacts/p4_l1_halfstep_step30_compare/secant_halfstep_step30/data/progress.jsonl"),
        "baseline": True,
    },
    {
        "name": "affine_halfstep_step30_prefix15",
    ...
    {
        "name": "affine_refined_step15",
        "label": "Affine States, Fixed-U Refined Lambda",
        "progress": Path("artifacts/p4_l1_halfstep_step15_lambda_compare/affine_refined_step15/data/progress.jsonl"),
        "baseline": False,
    },
    {
        "name": "affine_window3_refined_partial_step15",
        "label": "Affine States Window-3, Refined Lambda, Max5 Partial",
        "progress": Path("artifacts/p4_l1_halfstep_step15_lambda_compare/affine_window3_refined_partial_step15/data/progress.jsonl"),
        "baseline": False,
    },
    {
        "name": "power_p3_refined_partial_step15",
        "label": "Power Basis P3, Refined Lambda, Max5 Partial",
        "progress": Path("artifacts/p4_l1_halfstep_step15_lambda_compare/power_p3_refined_partial_step15/data/progress.jsonl"),
        "baseline": False,
    },
    {
        "name": "power_p3_equal_refined_partial_step15",
        "label": "Power Basis P3 Equal Split, Refined Lambda, Max5 Partial",
        "progress": Path("artifacts/p4_l1_halfstep_step15_lambda_compare/power_p3_equal_refined_partial_step15/data/progress.jsonl"),
        "baseline": False,
    },
    {
        "name": "power_p3_equal_refined_tol1e10_max20_partial_step15",
        "label": "Power Basis P3 Equal Split, Refined Lambda, Tol1e-10 Max20 Partial",
        "progress": Path("artifacts/p4_l1_halfstep_step15_lambda_compare/power_p3_equal_refined_tol1e10_max20_partial_step15/data/progress.jsonl"),
        "baseline": False,
    },
    {
        "name": "power_window2_p2_refined_partial_step15",
        "label": "Power+Window Basis W2 P2, Refined Lambda, Max5 Partial",
        "progress": Path("artifacts/p4_l1_halfstep_step15_lambda_compare/power_window2_p2_refined_partial_step15/data/progress.jsonl"),
        "baseline": False,
    },
    {
        "name": "power_p2_refined_partial_step15",
        "label": "Power Basis P2, Refined Lambda, Max5 Partial",
        "progress": Path("artifacts/p4_l1_halfstep_step15_lambda_compare/power_p2_refined_partial_step15/data/progress.jsonl"),
        "baseline": False,
    },
)
...
def main() -> None:
    ...
    lines: list[str] = []
    lines.append("# P4(L1) Half-Step Step-15 Lambda Comparison")
    lines.append("")
    lines.append("This report compares half-step startup prefixes to accepted step 15:")
    lines.append("- secant predictor")
    lines.append("- affine reduced Newton on previous continuation states with current fine `lambda_i`")
    lines.append("- affine reduced Newton on previous continuation states with fixed-`U` refined lambda")
    lines.append("- affine reduced Newton on a 3-state sliding window with fixed-`U` refined lambda, max 5 projected iterations, no fallback")
    lines.append("- reduced Newton in the normalized power basis of the last increment with `p=3`, fixed-`U` refined lambda, max 5 projected iterations, no fallback")
    lines.append("- reduced Newton in the normalized power basis of the last increment with `p=3` and equal-split seed, fixed-`U` refined lambda, max 5 projected iterations, no fallback")
    lines.append("- reduced Newton in the normalized power basis of the last increment with `p=3` and equal-split seed, fixed-`U` refined lambda, reduced tol `1e-10`, max 20 projected iterations, no fallback")
    lines.append("- reduced Newton in the last-two-increments power basis with `p=2` (`Δ_i`, `Δ_i^2`, `Δ_{i-1}`, `Δ_{i-1}^2`), fixed-`U` refined lambda, max 5 projected iterations, no fallback")
    lines.append("- reduced Newton in the normalized power basis of the last increment with `p=2`, fixed-`U` refined lambda, max 5 projected iterations, no fallback")
    lines.append("")
    ...
    lines.append("")
    lines.append("`Extra Overhead` is `Runtime - Init - sum(accepted-step wall times)`.")
    lines.append("")
    ...
    lines.append("")
    lines.append("| Case | Step | Predictor Kind | Predictor Error | Predictor Wall [s] | `u_init -> u_final` | Secant Ref | Ratio | Dev Mismatch | Secant Ref Dev | Dev Ratio | Lambda Ini | Lambda Final |")
    lines.append("| --- | ---: | --- | --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |")
    for case in predictor_cases:
        for obj in case["steps"]:
            ...
            pwall = float(obj.get("predictor_wall_time", math.nan)) if obj.get("predictor_wall_time") is not None else math.nan
            lines.append(
                f"| {case['label']} | {int(obj['accepted_step'])} | {obj.get('predictor_kind', '')} | "
                f"{obj.get('predictor_error', '')} | {pwall:.3f} | {disp:.3f} | {sec_disp:.3f} | {_safe_ratio(disp, sec_disp):.3f} | "
