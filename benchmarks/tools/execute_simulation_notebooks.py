#!/usr/bin/env python
"""Execute benchmark simulation notebooks through the standalone C/PETSc runner."""
...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--cases", nargs="*", default=None, help="Optional benchmark directory names to execute.")
    parser.add_argument("--run-mode", default="run", choices=("run", "auto", "reuse"))
    parser.add_argument("--execution-profile", default="smoke", choices=("smoke", "benchmark", "full"))
    parser.add_argument("--mpi-ranks", type=int, default=2)
    parser.add_argument("--executed-dir", type=Path, default=ROOT / ".local" / "tmp" / "notebook_simulation_execution")
...
def _rewrite_assignment(source: str, name: str, value: object) -> str:
    ...
    if not pattern.search(source):
        return source
    return pattern.sub(f"{name} = {_python_literal(value)}", source, count=1)


def _prepare_notebook(notebook_path: Path, *, run_mode: str, execution_profile: str, mpi_ranks: int | None):
    notebook = nbformat.read(notebook_path, as_version=4)
    for cell in notebook.cells:
        if cell.cell_type != "code":
            continue
        source = str(cell.get("source", ""))
        source = _rewrite_assignment(source, "RUN_MODE", run_mode)
        source = _rewrite_assignment(source, "EXECUTION_PROFILE", execution_profile)
        source = _rewrite_assignment(source, "MPI_RANKS", mpi_ranks)
        cell["source"] = source
    return notebook
...
def main() -> None:
    ...
    for case_toml in _selected_case_tomls(args.cases):
        notebook_path = case_toml.parent / "simulation.ipynb"
