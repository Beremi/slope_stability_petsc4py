...
def _import_cell(case_dir_name: str):
    return _code_cell(
        ...
        CASE_TOML = ROOT / "benchmarks" / "cases" / "{case_dir_name}" / "case.toml"
...
def _common_intro_cell(meta: dict[str, object], *, role: str):
    lines = [
        ...
        "",
        f"- Case config: `benchmarks/cases/{meta['case_dir_name']}/case.toml`",
        f"- Asset: `{meta['asset']}`",
...
def generate_all(benchmarks_dir: Path = BENCHMARKS_DIR) -> list[Path]:
    ...
    for case_toml in sorted(benchmarks_dir.glob("*/case.toml")):
        generated.extend(generate_notebooks_for_case(case_toml))
    return generated


def generate_notebooks_for_case(case_toml: Path) -> list[Path]:
    ...
    if legacy_path.exists():
        legacy_path.unlink()
    return [simulation_path, visualisation_path]


def generate_notebooks(benchmarks_dir: Path = BENCHMARKS_DIR) -> list[Path]:
    return generate_all(benchmarks_dir)
