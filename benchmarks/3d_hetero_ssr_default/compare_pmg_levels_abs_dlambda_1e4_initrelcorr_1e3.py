...
DEFAULT_REPORT_PATH = BENCHMARK_DIR / "comparisons_pmg_levels_abs_dlambda_1e4_initrelcorr_1e3.md"
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Run PMG level comparison for P2(L1..L5), P4(L1), and P4(L2) with "
            "|delta lambda| < 1e-4 and init relative correction < 1e-3."
        )
    )
    ...
    parser.add_argument(
        "--report-title",
        type=str,
        default="PMG Level Comparison: |Δlambda| < 1e-4, Init Relative Correction < 1e-3",
    )
...
def _case_specs(*, artifact_root: Path, case_keys: list[str] | tuple[str, ...]) -> list[CaseSpec]:
    ...
    for key in requested:
        ...
        specs.append(
            CaseSpec(
                ...
                hierarchy=str(entry["hierarchy"]),
                mesh_path=Path(entry["mesh_path"]).resolve(),
                elem_type=str(entry["elem_type"]),
                node_ordering=str(entry["node_ordering"]),
                coarse_mesh_path=(
                    None if entry["coarse_mesh_path"] is None else Path(entry["coarse_mesh_path"]).resolve()
                ),
                out_dir=runs_root / key,
                config_path=commands_root / f"{key}.json",
                petsc_opts=tuple(str(opt) for opt in tuple(entry["petsc_opts"])),
            )
        )
    return specs
...
def _case_command(*, spec: CaseSpec, omega_target: float, mpi_ranks: int) -> list[str]:
    ...
    if spec.coarse_mesh_path is not None:
        cmd.extend(["--pmg_coarse_mesh_path", str(spec.coarse_mesh_path)])
...
def _write_report(
    *,
    ...
    lines: list[str] = [
        ...
        "- `P2` PETSc opts: " + ", ".join(f"`{opt}`" for opt in P2_PETSC_OPTIONS),
        "- `P4` PETSc opts: " + ", ".join(f"`{opt}`" for opt in P4_PETSC_OPTIONS),
...
def main() -> None:
    ...
    specs = _case_specs(artifact_root=artifact_root, case_keys=args.case_keys)
    for spec in specs:
